...
## 클래스 인라인하기 \_ Inline Class

1. 배경

   - 클래스 인라인하기는 클래스 추출하기를 거꾸로 돌리는 리팩터링이다.
   - 역할을 옮기는 리팩터링 후, 특정 클래스에 남은 역할이 거의 없을 때 가장 많이 사용되는 클래스로 흡수시킨다.
   - 두 클래스의 기능을 지금과 다르게 분배하고 싶을 때도 클래스를 인라인한다. 합친 다음 새로운 클래스를 추출하는 게 쉬울 수도 있기 때문
...
## 위임 숨기기 \_ Hide Delegate

1. 배경

   - 모듈화 설계를 제대로 하는 핵심은 캡슐화다.
   - 캡슐화는 모듈들이 시스템의 다른 부분에 대해 알아야 할 내용을 줄여준다.
   - 서버 객체의 필드가 가리키는 객체(위임 객체)의 메서드를 호출하려면 클라이언트는 이 위임 객체를 알아야 하고, 위임 객체의 인터페이스 변경 시 호출하는 모든 클라이언트는 수정해야 한다.
...
## 중개자 제거하기 \_ Remove Middle Man

1. 배경

   - 클라이언트가 위임 객체의 또 다른 기능을 사용하고 싶을 때마다 서버에 위임 메서드를 추가해야 한다.
   - 서버 클래스가 그저 중개자 역할로 전락하여, 차라리 클라이언트가 위임 객체를 직접 호출하는 게 나을 수 있다.
   - 시스템이 바뀌면 '적절하다'의 기준도 바뀌기 마련이다. 위임 숨기기와 중개자 제거하기 리팩터링을 통해 적절한 지점을 찾아간다.
...
## 알고리즘 교체하기 \_ Substitute Algorithm

1. 배경

   - 더 간명한 방법을 찾아내면 복잡한 기존 코드를 간면한 방식으로 고친다.
   - 문제를 더 확실히 이해하고 훨씬 쉽게 해결하는 방법을 찾았을 때 알고리즘을 바꾼다.
